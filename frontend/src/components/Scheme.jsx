...
const SchemeCard = ({ item, handleViewDetails, isVisible }) => (
  <div
    className={`scheme-card bg-slate-800/50 backdrop-blur-sm rounded-xl shadow-lg border border-slate-700 p-6 flex flex-col justify-between h-full transform transition-all duration-500 hover:-translate-y-2 hover:border-cyan-400/50 overflow-hidden relative ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
  >
    <div className="absolute top-0 left-0 h-1 w-full bg-gradient-to-r from-cyan-400 to-blue-500 opacity-70"></div>
    <div className="space-y-4">
      <h3 className="text-xl font-semibold text-slate-100 pt-2">{item.title}</h3>
