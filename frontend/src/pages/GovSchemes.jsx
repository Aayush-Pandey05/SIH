...
const GovSchemes = () => {
  ...
  return (
    <div
      className={`min-h-screen font-sans text-slate-100 bg-slate-950 bg-[radial-gradient(at_50%_50%,_#1e293b_0%,_#020617_75%)] p-8 transition-all duration-500 ease-in-out ${
        isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'
      }`}
    >
      <div className="text-center mb-8 border-b border-white border-opacity-10 pb-6 mt-16">
        <h1 className="text-4xl font-bold mb-2 text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-sky-500">
          Government Scheme Integration
        </h1>
        <p className="text-lg text-slate-400">
          Based on your location and roof size, here are the government subsidies and schemes you are eligible for.
        </p>
      </div>

      <div className="flex flex-col md:flex-row items-center justify-between p-6 bg-slate-800 rounded-xl shadow-lg mb-6">
        <h3 className="text-xl font-semibold text-slate-300 md:mb-0 mb-4 whitespace-nowrap">
          Generate Your Report Template for government schemes:
        </h3>
        <button className="px-6 py-2 bg-sky-500 text-white font-semibold rounded-lg shadow-lg hover:bg-sky-600 transition-colors duration-200 ease-in-out uppercase tracking-wide md:w-auto w-full">
          Generate
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {schemes.map((scheme, index) => (
          <div
            key={index}
            className="bg-slate-800 rounded-xl shadow-lg p-6 flex flex-col justify-between transform transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl"
